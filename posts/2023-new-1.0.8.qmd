...
of `ETA`s to capture into the `@etas` option. For examples
...
`ETA3` in the simulated output. **Note** that the syntax 
_resembles_ what you might right in NONMEM where `last`
is the total number of `ETA`s in the problem. 

Also note that `ETA` naming resembles what you would get from 
...
a mrgsolve version of that run as `101.cpp` or `101.mod`.
...
Also, there will be an error if you ask for `ETA`s from the 
