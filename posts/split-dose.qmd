...
```{r, code = readLines(here("posts/model/split-dose-1.mod")), eval = FALSE}
...
```{r}
#| echo: false
dose2 <- mutate(dose, cmt = 2)
out2 <- mrgsim(mod, dose2, recsort = 3)
out1 <- out
```
...
We have to use the `nm-vars` plugin to get the `F1` syntax; otherwise, we could 
have set `F_depot1` instead.
...
```{r, eval = FALSE, code = readLines(here("posts/model/split-dose-2.mod"))}
...
compare <- dplyr::bind_rows(
  ...
  mutate(out2, model = "200 mg all to depot2"), 
  mutate(out3, model = "200 mg split depot1/depot2")
)
...
```{r, eval = FALSE, code = readLines(here("posts/model/split-dose-3.mod"))}
