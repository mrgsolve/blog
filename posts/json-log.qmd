...
well. This will naturally slow down your model a little bit; but so far it's 
been difficult to detect a meaningful change in simulation run time with the 
logging on or off. And, of course, this will depend on your application and 
there will be some limit. But especially if you are only _logging_ events where
...
but might not be that easy. When I was collecting information in C++ STL 
...
worth it for the extra flexibility. Nevertheless, if you are _only_ saving
outputs, the JSON solution works great.
