...
In **Part 1** we create a json object
...
In **Part 2** we write the completed `json` array to a file
...
- Then we write the data to file
  ...
  - Then serialize the `json` array, writing it to the file
  - Finally close the file then
  - Clear the `json` array for safety
...
json code for the first two Ctrough points looks like this.
...
I've made the code "pretty" to show you; in the file it won't look as nicely 
formatted as this.
