...
to consider when using this method for an infusion.
...
2. Calculate concentration (say `CP`) in the `$DES` block. 
3. Update `Cmax` to the value of `CP` calculated in `$DES` whenever `CP` is 
   greater than the current value of `Cmax`. 
...
of the concentration-time profile around Cmax to understand when to stop
increasing and start decreasing. The solver will explore this part of the curve
whether or not you ask for simulated output at that time. If, instead, you
simulated, say, every hour and then took the maximum `CP` over the dosing
interval, it will be approximate since you likely didn't ask for an output time
exactly at the time of the maximum concentration (Tmax).
...
resolution on the Cmax calculation if we only looked at `CP` when it appears
on an output record. This means we are relying on the ODEs to give us the 
true value. 
...
required for a good approximation in certain problems. 
...
We can verify this behavior by reducing `hmax`, the maximum step 
size. When `hmax` is smaller, the ODE solver takes smaller steps and the 
overshoot won't be as severe. 
...
out. In the meantime, I'll continue to use the approximate for most cases. 
