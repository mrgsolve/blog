...
of the concentration-time profile around Cmax to understand when the
concentration stops increasing and starts to decrease. increasing and start
decreasing. The solver will explore this part of the curve whether or not you
ask for simulated output at the time of Cmax (Tmax) and you can get the true
Cmax by logging the concentration at Tmax. If, instead, you simulated, say,
every hour and then took the maximum `CP` over the dosing interval, it will be
and approximate Cmax since you likely didn't ask for an output time exactly at
Tmax.
...
resolution of the simulated output; if we only look at `CP` on output records, 
we'll get a poor approximation of Cmax. This just means we _have_ to use the 
ODE-derived Cmax to get a the true value back. 
